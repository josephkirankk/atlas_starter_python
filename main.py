...
my_collection_config = db["config"]
#my_collection_config.drop()  

# config collection contains name and value pair documents. check if the key "config" exists and then insert or update it
x = my_collection_config.find_one({"name": "ip"})
# get the value
print("config value is %x" %(x["value"]))

if my_collection_config.find_one({"name": "ip"}) is None:
  my_collection_config.insert_one({"name": "ip", "value": 1})
else:
  my_collection_config.update_one({"name": "ip"}, {"$inc": {"value": 1}})
  print("config value incremented")
print("\n")
